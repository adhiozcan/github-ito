package id.net.iconpln.apps.ito.helper;

/**
 * Created by Ozcan on 31/03/2017.
 */

public class Constants {
    // for login configuration
    public static final String IS_REMEMBER     = "app.ito.is_remember";
    public static final String SAVE_USER       = "app.ito.user_save";
    public static final String USERNAME        = "app.ito.username";
    public static final String PASSWORD        = "app.ito.password";
    public static final String NAMA_PETUGAS    = "app.ito.nama_petugas";
    public static final String KODE_PETUGAS    = "app.ito.kode_petugas";
    public static final String ID_UNIT_UP      = "app.ito.id_unit_up";
    public static final String ID_UNIT_UP_NAMA = "app.ito.id_unit_up_nama";
    public static final String ID_UNIT_UPI     = "app.ito.id_unit_upi";
    public static final String ID_UNIT_AP      = "app.ito.id_unit_ap";

    // for network purpose
    public static final String SOCKET_FAILURE         = "app.ito.socket_failure";
    public static final String SOCKET_FAILURE_PARSING = "app.ito.socket_failure_parsing";

    // for synchronize purpose
    public static final String SINKRONISASI_SUKSES  = "Sukses";
    public static final String SINKRONISASI_PROSES  = "Proses";
    public static final String SINKRONISASI_GAGAL   = "Gagal";
    public static final String SINKRONISASI_PENDING = "Pending";
}
